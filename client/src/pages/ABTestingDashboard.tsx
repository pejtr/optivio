...
export default function ABTestingDashboard() {
  const [metrics, setMetrics] = useState<VariantMetrics[]>([
    { variant: 'A', pageViews: 1240, ctaClicks: 89, conversions: 12, conversionRate: 0.97, ctr: 7.18 },
    { variant: 'B', pageViews: 1156, ctaClicks: 98, conversions: 15, conversionRate: 1.30, ctr: 8.48 },
    { variant: 'C', pageViews: 1089, ctaClicks: 102, conversions: 18, conversionRate: 1.65, ctr: 9.37 },
    { variant: 'D', pageViews: 1203, ctaClicks: 76, conversions: 8, conversionRate: 0.67, ctr: 6.32 },
  ]);

  const [winner, setWinner] = useState<'A' | 'B' | 'C' | 'D'>('C');

  const topVariant = metrics.reduce((prev, current) => 
    current.conversionRate > prev.conversionRate ? current : prev
  );

  const totalPageViews = metrics.reduce((sum, m) => sum + m.pageViews, 0);
  const totalConversions = metrics.reduce((sum, m) => sum + m.conversions, 0);
  const overallConversionRate = ((totalConversions / totalPageViews) * 100).toFixed(2);
          ...
          <Card className="bg-gradient-to-br from-green-500/20 to-emerald-500/20 border-green-500/30 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold">{topVariant.variant}</p>
              </div>
          ...
          <Card className="bg-slate-800/50 border-purple-500/20 p-6">
            ...
            <ul className="space-y-3 text-slate-300">
              <li className="flex gap-2">
                ...
                <span>Variant C has highest conversion rate at {topVariant.conversionRate.toFixed(2)}%</span>
              </li>
              <li className="flex gap-2">
